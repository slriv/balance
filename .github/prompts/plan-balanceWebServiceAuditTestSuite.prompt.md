...
Key properties: `field` = lexically scoped instance variable; `method` = sub with implicit `$self`; `ADJUST` = post-constructor validation; `:param` = settable via `new()`; `:isa(Base)` = inheritance; `:reader`/`:writer` for accessors; bare `field` is private. No `Exporter` — classes are consumed by instantiation, not import.
...
- HTTP-calling subs (`Sonarr`, `Plex`) are tested with `Test::MockModule` intercepting `HTTP::Tiny`
...
| `lib/Balance/PathMap.pm` | A | `v5.38`, `signatures`, `try`; add `reverse_translate_path`, `nas_roots` | `load_path_map`, `translate_path`, `reverse_translate_path`, `nas_roots` |
...
| `lib/Balance/WebClient.pm` | **B** | **New** — extract shared HTTP base class | `base_url()`, `_api_get($path)`, `_auth_headers()` (template method), HTTP instance cached across calls |
| `lib/Balance/Sonarr.pm` | **B** | Convert to `class :isa(Balance::WebClient)`; `api_key` in `field`; override `_auth_headers()`; CLI dispatch moves to `bin/sonarr_reconcile.pl` | `get_series`, `update_series_path`, `rescan_series`, `resolve_series_id`, `apply_plan` |
| `lib/Balance/Plex.pm` | **B** | Convert to `class :isa(Balance::WebClient)`; `token` in `field`; override `_auth_headers()`; CLI dispatch moves to `bin/plex_reconcile.pl` | `list_libraries`, `scan_path`, `empty_trash`, `resolve_library_id`, `apply_plan` |
...
### Phase 0 — Standards & Scaffolding *(complete)*
...
✅ 5. Retrofit `Sonarr` → Pattern B (`class :isa(Balance::WebClient)`); write `t/unit/Balance/Sonarr.t` with `Test::MockModule` on `HTTP::Tiny`
✅ 6. Retrofit `Plex` → Pattern B (same) + `Plex.t`; extract `Balance::WebClient` base class; write `t/unit/Balance/WebClient.t`
...
| `lib/Balance/Sonarr.pm` | Retrofit to Pattern B (`:isa(Balance::WebClient)`); add `audit`, `repair` sub-commands |
