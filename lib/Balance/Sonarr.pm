...
class Balance::Sonarr {  ## no critic (Modules::RequireEndWithOne)
    ...
    ADJUST {
        ...
        $_client = WebService::Arr::Sonarr->new(baseurl => $base_url, api_key => $api_key);
    }
    ...
    method rescan_series($series_id) {
        return $_client->command('RescanSeries', seriesId => int($series_id));
    }

    method refresh_series($series_id) {
        return $_client->command('RefreshSeries', seriesIds => [int($series_id)]);
    }

    # Fetch series by id, update path field, PUT back. Returns updated series object.
    method update_series_path($series_id, $new_path) {
        my $series = $_client->series_by_id($series_id);
        $series->{path} = $new_path;
        return $_client->update_series($series_id, $series);
    }

    method get_root_folders() {
        return $_client->root_folders();
    }
    ...
    sub _cli_usage($exit_code, $error = undef) {
        ...
        print STDERR <<~'USAGE';
            Usage: balance_sonarr <command> [options]
            ...
            Examples:
                balance_sonarr series
                balance_sonarr rescan --series-id=123
                balance_sonarr refresh --series-id=123
                balance_sonarr apply --report-file=var/reconcile-plan.json
                balance_sonarr dry-run --report-file=var/reconcile-plan.json
                balance_sonarr audit --report-file=var/sonarr-audit-report.json
                balance_sonarr repair-dry-run --report-file=var/sonarr-audit-report.json
        USAGE
        exit $exit_code;
    }
}
...
=head1 SYNOPSIS
  ...
  my $series = $sonarr->get_series();
  $sonarr->update_series_path($series_id, $new_path);
...
workflow for the Balance media management tool. It delegates HTTP transport
to L<WebService::Arr::Sonarr> and wraps it with Balance-specific workflow
logic (audit, repair, apply-plan). It reads a Balance reconcile plan file
and updates Sonarr series paths after media moves, with optional dry-run
preview.
